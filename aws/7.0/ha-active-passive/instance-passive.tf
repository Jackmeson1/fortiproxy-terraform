...
resource "aws_instance" "fpxpassive" {
  depends_on           = [aws_instance.fpxactive]
  ami                  = var.license_type == "payg" ? var.fpxvmpaygami[var.region] : var.fpxvmbyolami[var.region]
  instance_type        = var.size
  availability_zone    = var.az2
  key_name             = var.keyname
  user_data            = data.template_file.passiveFortiProxy.rendered
