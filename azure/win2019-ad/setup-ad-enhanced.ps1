powershell -ExecutionPolicy Unrestricted -Command "
  ...
  try {
    ...
    New-ADOrganizationalUnit -Name 'Linux Clients' -Path 'DC=example,DC=com'
    ...
    New-ADGroup -Name 'Linux-Admins' -GroupScope Global -GroupCategory Security -Path 'DC=example,DC=com'
    New-ADGroup -Name 'SSH-Users' -GroupScope Global -GroupCategory Security -Path 'DC=example,DC=com'
    
    # Create Admin Users
    New-ADUser -Name 'admin.it' -UserPrincipalName 'admin.it@${var.domain_name}' -SamAccountName 'admin.it' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Admin Accounts,DC=example,DC=com' -GivenName 'IT' -Surname 'Administrator' -PasswordNeverExpires $true
    New-ADUser -Name 'admin.security' -UserPrincipalName 'admin.security@${var.domain_name}' -SamAccountName 'admin.security' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Admin Accounts,DC=example,DC=com' -GivenName 'Security' -Surname 'Administrator' -PasswordNeverExpires $true
    ...
    New-ADUser -Name 'svc.ldap' -UserPrincipalName 'svc.ldap@${var.domain_name}' -SamAccountName 'svc.ldap' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Service Accounts,DC=example,DC=com' -Description 'LDAP Service Account' -PasswordNeverExpires $true -KerberosEncryptionType AES256
    New-ADUser -Name 'svc.krb' -UserPrincipalName 'svc.krb@${var.domain_name}' -SamAccountName 'svc.krb' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Service Accounts,DC=example,DC=com' -Description 'Kerberos Service Account' -PasswordNeverExpires $true -KerberosEncryptionType AES256
    New-ADUser -Name 'svc.backup' -UserPrincipalName 'svc.backup@${var.domain_name}' -SamAccountName 'svc.backup' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Service Accounts,DC=example,DC=com' -Description 'Backup Service Account' -PasswordNeverExpires $true
    New-ADUser -Name 'svc.monitoring' -UserPrincipalName 'svc.monitoring@${var.domain_name}' -SamAccountName 'svc.monitoring' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Service Accounts,DC=example,DC=com' -Description 'Monitoring Service Account' -PasswordNeverExpires $true
    
    # Create test users for Linux authentication
    New-ADUser -Name 'linux.admin' -UserPrincipalName 'linux.admin@${var.domain_name}' -SamAccountName 'linux.admin' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Linux Clients,DC=example,DC=com' -GivenName 'Linux' -Surname 'Administrator' -uidNumber 10001 -gidNumber 10001
    New-ADUser -Name 'linux.user1' -UserPrincipalName 'linux.user1@${var.domain_name}' -SamAccountName 'linux.user1' -AccountPassword (ConvertTo-SecureString '${var.admin_password}' -AsPlainText -Force) -Enabled $true -Path 'OU=Linux Clients,DC=example,DC=com' -GivenName 'Linux' -Surname 'User1' -uidNumber 10002 -gidNumber 10001
    ...
    setspn -A host/windc2019.${var.domain_name} svc.krb
    ...
    $cert = New-SelfSignedCertificate -DnsName 'windc2019.${var.domain_name}' -CertStoreLocation 'cert:\LocalMachine\My' -KeyExportPolicy Exportable -KeySpec KeyExchange -KeyLength 2048 -KeyAlgorithm RSA -HashAlgorithm SHA256
